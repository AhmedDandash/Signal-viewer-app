...
from PyQt5.QtWidgets import QFileDialog, QMessageBox, QColorDialog, QListWidgetItem, QMessageBox, QDialogButtonBox
import wfdb
import numpy as np
import sys
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def init_ui(self):
        ...
        self.linkButton_4.clicked.connect(self.link_graphs)
...
# ************************************** HELPER FUNCTIONS **************************************
    ...
    def fill_list1(self): #list to show channels
    ...
    def fill_list2(self):#list to show channels
    ...
    def sudden_appearing(self, graph, j): #to make the signal appear when the channel move
...
# ************************************** Plot Graphs **************************************
    ...
    def plot_graph_signal(self):
        if len(self.signals[self.get_graph_name()]) == 1:  # first time to plot
    ...
    def plot_common_linked_signal(self):
        for i, graph_name in enumerate(["graph1", "graph2"]):
            if len(self.signals[graph_name]) == 1:  # first time to plot
    ...
    def updating_graphs(self, graph: str): #dynamic view
        for i, signal in enumerate(self.signals[graph]):
            (time, data), end_ind = signal
            signal_line = self.signals_lines[graph][i]
            #Extracts a subset of time and data based on the determined end index and an additional data index.
            X = time[:end_ind + self.data_index[graph]]
            Y = data[:end_ind + self.data_index[graph]]
            self.signals[graph][i] = [(time, data), end_ind + self.data_index[graph]]
            #sets the x-range of the graph to be a fraction of the total time, ensuring a dynamic view.
            if (X[-1] < time[-1] / 5):
...
# ************************************** Transfer signals **************************************
    ...
    def transfer_signal(self):
        ...
        elif self.get_graph_name() == "graph2":# from graph2 --> graph1
            curr_channel_ind = self.channels_selected["graph2"]
            self.transfer_data_between_globals(curr_channel_ind)
        else:
            self.show_error_message("Can't transfer, specify a graph!")# from no graph selected
...
# ************************************** Snapshoot and PDF Report **************************************
    ...
    def add_snapshots_to_pdf(self, pdf):
        ...
        for graph_image in snap_data:
            ...
            pdf.cell(200, 10, txt=image_name)
            pdf.ln(10)
    ...
    def add_statistics_tables(self):
        ...
        for graph_name in graph_names:
            ...
            if statistics:
                self.pdf.cell(200, 10, txt=f"Statistics for {graph_name}")
